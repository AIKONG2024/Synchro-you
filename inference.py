...
def make_df_angle(pose_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for body_parts1, body_parts2 in angle:
        vec_mat1 = pose_df.iloc[:, body_parts[body_parts1][0]*3:body_parts[body_parts1][0]*3+3].values \
                   - pose_df.iloc[:, body_parts[body_parts1][1]*3:body_parts[body_parts1][1]*3+3].values
        vec_mat2 = pose_df.iloc[:, body_parts[body_parts2][0]*3:body_parts[body_parts2][0]*3+3].values \
                   - pose_df.iloc[:, body_parts[body_parts2][1]*3:body_parts[body_parts2][1]*3+3].values
...
def embedding(angle_df: pd.DataFrame):
    ...
    input_tensor = torch.tensor(angle_df.values).unsqueeze(0).to(torch.float32).to(device)
    with torch.no_grad():
...
def calculate_similarity(vector1, vector2):
    ...
    distance = torch.nn.functional.pairwise_distance(norm1, vector2, p=2)
